...
class PymixMem:
    ...
    def get_pdbinfo(self, inp:dict):
        pdbinfo = []
        for k in inp.keys():
            u = mda.Universe(k)  # Load each lipid structure as a universe
            print(f'Reading {k}, total {u.atoms.n_atoms} atoms...')
    
            # Find the longest axis and initially align it with the Z-axis
            dist, ia, ib = -999999, 0, 0
            for i, atom_i in enumerate(u.atoms):
                for j, atom_j in enumerate(u.atoms):
                    if i >= j:
                        continue
                    r = calc_bonds(atom_i.position, atom_j.position, box=None)
                    if r > dist:
                        dist, ia, ib = r, i, j
    
            vec = u.atoms.positions[ia] - u.atoms.positions[ib]
            vec /= np.linalg.norm(vec)
            angle = np.arccos(vec[2]) * 180.0 / np.pi
            raxis = np.cross(vec, [0, 0, 1])
            rotateby(angle, direction=raxis, ag=u.atoms)(u.trajectory.ts)
    
            # Check for N and P atoms, use them to confirm orientation if present
            reference_atoms = u.select_atoms('name N*')
            if len(reference_atoms) == 0:
                reference_atoms = u.select_atoms('name P*')
                if len(reference_atoms) == 0:
                    reference_atoms = None
    
            if reference_atoms:
                reference_position = reference_atoms.center_of_mass()
                center_of_mass = u.atoms.center_of_mass()
                if reference_position[2] < center_of_mass[2]:
                    u.atoms.rotateby(180, [1, 0, 0])
